 ...
 * 项目中 所有接口请求归属该类
 */
final class ServiceApiManagerOffline extends com.lt.ServiceApiManager {
    ...
    public Flowable<String> customRequest(@Body Object request, @Header("custom_type") int type) {
        return readFromDb(new com.lt.LocalInterceptor().filterPath("/" + CustomUrlConfig.PATH_BASE, type + ""), request)
                .flatMap(new DecodeBodyFunction<String>() {
                }).subscribeOn(RxSchedulers.request())
                .observeOn(RxSchedulers.main());
    }
