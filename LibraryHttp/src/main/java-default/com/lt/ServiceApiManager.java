...
public class ServiceApiManager {
    ...
    public Flowable<String> requestFlyerInfo(WorkerInfoRequest request) {
        return serviceApi.requestFlyerInfo(request)
                .flatMap(new DecodeBodyFunction<String>() {
                }).subscribeOn(RxSchedulers.request())
...
//                .observeOn(RxSchedulers.main());
    ...
    Call<HttpEntity> requestUploadPic(@Header("token") String userToken, @Part("param") RequestBody param,
    ...
    public Call<HttpEntity> uploadLogFile(@Header("token") String userToken, @Header("Blade-Auth") String token, @Header("sign") String sign, @Body RequestBody uploadFile) {
        return serviceApi.uploadLogFile(token, sign, uploadFile);
    }
