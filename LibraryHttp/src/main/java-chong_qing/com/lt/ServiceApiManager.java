...
public class ServiceApiManager {
    ...
    public static ServiceApiManager getInstance() {
        if (OfflineConfig.isOfflineMode())
            return ServiceApiManagerOffline.getInstance();
        if (manager == null)
    ...
    public Flowable<String> requestLedgerGrid(GridRequest request) {
        return serviceApi.requestLedgerGrid(request)
                .flatMap(new DecodeBodyFunction<String>() {
                })
                .subscribeOn(RxSchedulers.request())
                .observeOn(RxSchedulers.main());
     ...
     * @return 结果
     */
    public Call<HttpEntity> uploadFlyLog(@Header("Blade-Auth") String token, @Header("sign") String sign, @Body RequestBody uploadFile) {
        return serviceApi.uploadFlyLog(token, sign, uploadFile);
    }
    /**
     * 获取交跨点信息
     *
     ...
     * @return 响应重命名结果
     */
    public Flowable<String> requestFlightCrossInfo(FlightCrossInfoRequest request) {
        return serviceApi.flightCrossInfo(request)
                .flatMap(new DecodeBodyFunction<String>() {
                })
                .subscribeOn(RxSchedulers.request())
                .observeOn(RxSchedulers.main());
     ...
     * @return x
     */
    public Flowable<String> queryInspectNum(QueryInspectNumRequest request) {
        return serviceApi.queryInspectNum(request)
                .flatMap(new DecodeBodyFunction<String>() {
                })
                .subscribeOn(RxSchedulers.request())
                .observeOn(RxSchedulers.main());
